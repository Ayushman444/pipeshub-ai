    ...
    async def init(self) -> bool:
        try:
            ...
            if not all([host, database, user]):
                self.logger.error("Missing required MariaDB configuration (host, database, user)")
                return False
    ...
    async def _get_table_options(
        ...
        if not self.data_source:
            return FilterOptionsResponse(
                ...
                limit=limit,
                has_more=False,
                message="MariaDB data source not initialized"
            )
        ...
        try:
            if not self.database_name:
                return FilterOptionsResponse(
                    ...
                    limit=limit,
                    has_more=False,
                    message="MariaDB database is not configured"
                )

            all_tables = []
            tables_response = await self.data_source.list_tables(database=self.database_name)
            if tables_response.success:
                ...
                return FilterOptionsResponse(
                    ...
                    limit=limit,
                    has_more=False,
                    message=tables_response.error or "Failed to fetch tables"
                )
